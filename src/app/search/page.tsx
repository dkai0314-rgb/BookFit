    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (data.items && data.items.length > 0) {
                ...
                alert("Ï∂îÏ≤ú Í≤∞Í≥ºÎ•?Ï∞æÏ? Î™ªÌñà?µÎãà?? ?§Î•∏ ?§Ïõå?úÎ°ú Í≤Ä?âÌï¥Î≥¥ÏÑ∏??");
            }
        } catch (error) {
            console.error("Search failed:", error);
            alert("?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§. ?†Ïãú ???§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="w-full max-w-2xl space-y-12 z-10">
                ...
                <div className="text-center space-y-4">
                    ...
                    <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-foreground break-keep leading-tight">
                        ?¥Îñ§ Ï±ÖÏùÑ <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">Ï∞æÍ≥† Í≥ÑÏã†Í∞Ä??</span>
                    </h1>
                    <p className="text-muted-foreground md:text-lg">
                        AIÍ∞Ä ?πÏã†??ÏßàÎ¨∏??Î∂ÑÏÑù????ÎßûÎäî Ï±ÖÏùÑ Ï∞æÏïÑ?úÎ¶Ω?àÎã§.
                    </p>
                </div>
                ...
                <div className="relative group">
                    ...
                    <div className="relative bg-card border border-border rounded-2xl shadow-lg flex items-center p-4 gap-4 focus-within:ring-2 focus-within:ring-primary/50 transition-all">
                        ...
                        <input
                            ...
                            placeholder="?? ?îÏ¶ò ÎßàÏùå???µÎãµ?úÎç∞ ?ÑÎ°úÍ∞Ä ?òÎäî ?êÏÑ∏??Ï∂îÏ≤ú?¥Ï§ò"
                            value={query}
                        ...
                        >
                            {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Í≤Ä??}
                        </Button>
                    </div>
                </div>
